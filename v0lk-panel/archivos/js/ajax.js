...
function ajax_load(url, tag_id, func, funcarg, silent, custlhtml) {
	...
	if (elem) {
		...
		if (!silent) elem.innerHTML = '<table><tr><td valign="center"><img src="archivos/imagen/ajax-loader.gif" alt="ajax-loader"></td><td valign="center"> <i>Loading...</i></td></tr></table>';
	...
	xmlhttp.onreadystatechange = function() {
		if (this.readyState == 4) {
			if (this.status == 200) {
				...
				if (!silent && elem) elem.innerHTML = '<table><tr><td><img src="archivos/imagen/ajax-error.png" alt="error"></td><td valign="center"> <font class="error">Error</font> : #' + this.status + ' </td></tr></table>';
...
function ajax_pload(url, pdata, tag_id, loader, callback, cbarg) {
	var elem = document.getElementById(tag_id);
	if (elem) {
		var ldr = '<table><tr><td valign="center"><img src="archivos/imagen/ajax-loader.gif" alt="ajax-loader"></td><td valign="center"> <i>Loading...</i></td></tr></table>';
		if (loader)
			ldr = loader;
		elem.innerHTML = ldr;
	}
	...
	xmlhttp.onreadystatechange = function() {
		if (this.readyState == 4) {
			if (this.status == 200) {
				...
				elem.innerHTML += '<br><table><tr><td><img src="archivos/imagen/error.png" alt="error"></td><td valign="center"> <font class="error">Error</font> : #' + this.status + ' </td></tr></table>';
			}
		}
	};
	xmlhttp.send(pdata);
